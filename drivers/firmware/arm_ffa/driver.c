		       ...
		       struct ffa_mem_ops_args *args)
{
	...
	if (ep_access_offset > max_fragsize ||
	    total_ep_mem > max_fragsize - ep_access_offset)
		return -ENOSPC;

	composite_offset = ffa_mem_desc_offset(buffer, args_nattrs,
					       drv_info->version);
	if (composite_offset > max_fragsize ||
	    sizeof(*composite) > max_fragsize - composite_offset)
		return -ENXIO;

	for (idx = 0; idx < args_nattrs; idx++) {
