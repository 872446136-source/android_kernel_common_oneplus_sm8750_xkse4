...
static enum dsq_type get_dsq_type(struct hmbird_dispatch_q *dsq)
{
	...
	if ((dsq->id & HMBIRD_DSQ_FLAG_BUILTIN) &&
		...
		((dsq->id & 0xff) < GDSQS_ID_BASE + MAX_GLOBAL_DSQS))
		return GLOBAL_DSQ;
...
static bool init_isolate_cpus(void)
{
	int cpu;
	...
	for (cpu = 0; cpu <= 4; cpu++)
		if (cpu_possible(cpu))
			cpumask_set_cpu(cpu, iso_masks.big);
	if (cpu_possible(5))
...
static int find_idx_from_task(struct task_struct *p)
{
	...
	if (tg && tg->css.cgroup && tg->css.cgroup->kn) {
		if (likely(tg->css.cgroup->kn->id < NUMS_CGROUP_KINDS))
...
inline u64 get_hmbird_cpu_util(int cpu)
{
	...
	if (!hrq || !hrq->prev_runnable_sum_fixed || !hrq->prev_window_size)
		return 0;

	prev_runnable_sum_fixed = READ_ONCE(*hrq->prev_runnable_sum_fixed);
	prev_window_size = READ_ONCE(*hrq->prev_window_size);
	divisor = prev_window_size >> SCHED_CAPACITY_SHIFT;
	if (!divisor)
		return 0;

	do_div(prev_runnable_sum_fixed, divisor);
...
static void init_level1_tg(struct cgroup *cgrp, struct task_group *tg)
{
	...
	if (cgrp->kn->id >= NUMS_CGROUP_KINDS)
		return;
...
static int dsq_global_idx(struct hmbird_dispatch_q *dsq)
{
	...
	id = dsq->id & 0xff;
	if (id < GDSQS_ID_BASE ||
	    id >= GDSQS_ID_BASE + MAX_GLOBAL_DSQS)
		return -1;
...
bool hmbird_prio_less(const struct task_struct *a,
		      const struct task_struct *b, bool in_fi)
{
	struct hmbird_entity *a_hse = get_hmbird_ts(a);
	struct hmbird_entity *b_hse = get_hmbird_ts(b);

	if (WARN_ON_ONCE(!a_hse || !b_hse))
		return a->pid > b->pid;

	return time_after(a_hse->runnable_at, b_hse->runnable_at);
}

static struct task_struct *pick_task_hmbird(struct rq *rq)
{
	return first_local_task(rq);
}
#endif
...
void hmbird_free(struct task_struct *p)
{
	...
	if (!list_empty(&hse->tasks_node))
		list_del_init(&hse->tasks_node);
	 ...
	 */
	if (test_bit(ffs(HMBIRD_TASK_OPS_PREPPED),
		     (unsigned long *)&hse->flags) ||
	    test_bit(ffs(HMBIRD_TASK_OPS_ENABLED),
		     (unsigned long *)&hse->flags)) {
		struct rq_flags rf;
		struct rq *rq;
...
int hmbird_check_setscheduler(struct task_struct *p, int policy)
{
	...
	if (hmbird_enabled() && hse && READ_ONCE(hse->disallow) &&
...
static void hmbird_setscheduler_prio(struct task_struct *p, int prio)
{
...
static void hmbird_ops_disable_workfn(struct kthread_work *work)
{
	...
	while ((p = hmbird_task_iter_next_filtered_locked(&sti))) {
		...
		SCHED_CHANGE_BLOCK(rq, p, DEQUEUE_SAVE | DEQUEUE_MOVE |
			...
			hmbird_setscheduler_prio(p, p->prio);
		}
...
static inline void set_audio_thread_sched_prop(struct task_struct *p)
{
	...
	if (css && css->cgroup && css->cgroup->kn &&
	    !strcmp(css->cgroup->kn->name, "audio-app"))
...
void __init init_sched_hmbird_class(void)
{
	...
	hmbird_ready = init_isolate_cpus();
#ifdef CONFIG_SMP
	if (!zalloc_cpumask_var(&idle_masks.cpu, GFP_KERNEL) ||
	    !zalloc_cpumask_var(&idle_masks.smt, GFP_KERNEL))
		hmbird_ready = false;
#endif
	...
	if (init_hmbird) {
		...
		RB_CLEAR_NODE(&init_hmbird->dsq_node.priq);
		INIT_LIST_HEAD(&init_hmbird->watchdog_node);
		INIT_LIST_HEAD(&init_hmbird->tasks_node);
		init_hmbird->sticky_cpu = -1;
		init_hmbird->holding_cpu = -1;
		init_hmbird->critical_affinity_cpu = -1;
		...
		init_hmbird->sched_class = &hmbird_sched_class;
		hmbird_set_sched_prop(&init_task, 0);
		list_add_tail(&init_hmbird->tasks_node, &hmbird_tasks);
	} else {
	...
	for_each_possible_cpu(cpu) {
		...
		if (!zalloc_cpumask_var(&get_hmbird_rq(rq)->cpus_to_kick,
				       GFP_KERNEL) ||
		    !zalloc_cpumask_var(&get_hmbird_rq(rq)->cpus_to_preempt,
				       GFP_KERNEL) ||
		    !zalloc_cpumask_var(&get_hmbird_rq(rq)->cpus_to_wait,
				       GFP_KERNEL)) {
			hmbird_ready = false;
			continue;
		}
