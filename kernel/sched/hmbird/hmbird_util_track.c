			  ...
			  u64 wallclock)
{
	...
	if (!max_freq) {
		srq->task_exec_scale = arch_scale_cpu_capacity(cpu);
		return;
	}
...
{
	if (!slim_walt_ctrl || cpu < 0 || cpu >= nr_cpu_ids)
		return 0;

	return slim_walt_cpu_util(cpu);
}
				...
				struct rq *rq, int event, u64 wallclock)
{
	...
	if (!slim_walt_ctrl || !p || !rq || !get_hmbird_ts(p))
		return;

	sts = &get_hmbird_ts(p)->sts;
	srq = &per_cpu(hmbird_sched_rq_stats, cpu_of(rq));
...
void hmbird_sched_init_task(struct task_struct *p)
{
	...
	hse = get_hmbird_ts(p);
	if (hse)
		memset(&hse->sts, 0, sizeof(hse->sts));
}
...
int sched_ravg_window_change(int frame_per_sec)
{
	...
	if (frame_per_sec <= 0 ||
	    frame_per_sec > (NSEC_PER_SEC >> SCX_SCHED_CAPACITY_SHIFT))
		return -EINVAL;
