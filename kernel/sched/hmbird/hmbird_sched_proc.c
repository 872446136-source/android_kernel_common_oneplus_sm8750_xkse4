...
static DEFINE_MUTEX(hmbird_proc_ctrl_mutex);
								...
								size_t count, loff_t *ppos)
{
	...
	if (set_proc_buf_val(file, buf, count, &enable))
		return -EFAULT;
	if (enable != 0 && enable != 1)
		return -EINVAL;

	mutex_lock(&hmbird_proc_ctrl_mutex);
	if (enable == !!hmbird_enabled()) {
		WRITE_ONCE(*pval, enable);
		mutex_unlock(&hmbird_proc_ctrl_mutex);
		return count;
	}

	WRITE_ONCE(sw_type, HMBIRD_SWITCH_PROC);
	ret = hmbird_ctrl(enable);
	if (ret) {
		WRITE_ONCE(*pval, !!hmbird_enabled());
		mutex_unlock(&hmbird_proc_ctrl_mutex);
		return ret;
	}
	WRITE_ONCE(*pval, enable);
			...
			const char __user *buf, size_t count, loff_t *ppos)
{
	...
	if (set_proc_buf_val(file, buf, count, &frame_per_sec))
		return -EFAULT;

	if (sched_ravg_window_change(frame_per_sec))
		return -EINVAL;
	*pval = frame_per_sec;
