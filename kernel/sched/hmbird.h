...
static inline void hmbird_notify_pick_next_task(struct rq *rq,
						...
						const struct sched_class *active)
{
	struct hmbird_rq *hmbird_rq;

	if (!hmbird_enabled())
		return;
	hmbird_rq = get_hmbird_rq(rq);
	if (WARN_ON_ONCE(!hmbird_rq))
		return;
#ifdef CONFIG_SMP
	/*
	 ...
	 * resched.
	 */
	smp_store_release(&hmbird_rq->pnt_seq, hmbird_rq->pnt_seq + 1);
#endif
...
hmbird_next_active_class(const struct sched_class *class)
{
	class++;
...
#define hmbird_for_active_class_range(class, _from, _to)			\
	for (class = (_from); class != (_to);				\
	     class = hmbird_next_active_class(class))

#define hmbird_for_each_active_class(class)				\
	hmbird_for_active_class_range(class, __sched_class_highest,	\
 ...
 */
#define hmbird_for_balance_class_range(class, prev_class, end_class)	\
	hmbird_for_active_class_range(class,				\
		(prev_class) > &hmbird_sched_class ?			\
...
static inline u16 hmbird_task_util(struct task_struct *p)
{
	...
	return p->sched_class == &hmbird_sched_class && entity ?
		entity->sts.demand_scaled : 0;
}
