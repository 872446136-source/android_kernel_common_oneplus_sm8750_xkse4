 ...
 * bit 0 - 28: remaining parts to complete this folio
 */
#define Z_EROFS_ONLINEFOLIO_EIO		30
#define Z_EROFS_ONLINEFOLIO_DIRTY	29
...
static void z_erofs_onlinefolio_end(struct folio *folio, int err, bool dirty)
{
	...
	do {
		...
		DBG_BUGON(orig <= 0);
		v = dirty << Z_EROFS_ONLINEFOLIO_DIRTY;
		v |= (orig - 1) | (!!err << Z_EROFS_ONLINEFOLIO_EIO);
	} while (atomic_cmpxchg((atomic_t *)&folio->private, orig, v) != orig);

	if (v & (BIT(Z_EROFS_ONLINEFOLIO_DIRTY) - 1))
		return;
	folio->private = 0;
	if (v & BIT(Z_EROFS_ONLINEFOLIO_DIRTY))
		flush_dcache_folio(folio);
	folio_end_read(folio, !(v & BIT(Z_EROFS_ONLINEFOLIO_EIO)));
}
...
repeat:
	...
	if (likely(folio->mapping == mc)) {
		...
		if (likely(folio->private == (void *)pcl)) {
	...
	folio_put(folio);
out_allocfolio:
	page = __erofs_allocpage(&f->pagepool,
				 gfp | __GFP_NOFAIL, true);
	spin_lock(&pcl->lockref.lock);
